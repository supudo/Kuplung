...
int Kuplung::run() {
	...
	while(this->gameIsRunning) {
		...
		glViewport(
				0, 0, Settings::Instance()->SDL_Window_Width, Settings::Instance()->SDL_Window_Height);
		glClearColor(Settings::Instance()->guiClearColor.r,
								 Settings::Instance()->guiClearColor.g,
								 Settings::Instance()->guiClearColor.b,
								 Settings::Instance()->guiClearColor.w);
	...
	if(SDL_Init(SDL_INIT_EVERYTHING) < 0) {
		...
		this->gWindow =
				SDL_CreateWindow(WINDOW_TITLE,
												 ...
												 Settings::Instance()->SDL_Window_Flags | SDL_WINDOW_ALLOW_HIGHDPI);
			...
			if(!this->glContext) {
				...
				if(SDL_GL_MakeCurrent(this->gWindow, this->glContext) < 0) {
					...
					this->managerUI->init(
							...
							std::bind(&Kuplung::guiProcessImportedFile,
												this,
												std::placeholders::_1,
												std::placeholders::_2),
							...
							std::bind(&Kuplung::guiSceneExport,
												this,
												std::placeholders::_1,
												std::placeholders::_2,
												std::placeholders::_3,
...
#	endif
	...
	CFURLRef appUrlRef;
	appUrlRef =
			CFBundleCopyResourceURL(CFBundleGetMainBundle(), CFSTR("Kuplung_Settings.ini"), NULL, NULL);
...
void Kuplung::onEvent(SDL_Event* ev) {
	...
	if(!this->managerUI->isMouseOnGUI() && !this->managerUI->isParsingOpen &&
		...
		if(this->managerControls->keyPressed_DELETE && this->sceneSelectedModelObject > -1 &&
			 this->meshModelFaces.size() > 0)
			this->guiModelDelete(this->sceneSelectedModelObject);

		if(this->sceneSelectedModelObject > -1 && this->meshModelFaces.size() > 0)
			this->meshModelFaces[static_cast<size_t>(this->sceneSelectedModelObject)]->Setting_EditMode =
					this->managerControls->keyPresset_TAB;
		...
		if(!ImGuizmo::IsUsing() && this->managerControls->mouseButton_LEFT) {
			if(this->managerControls->keyPresset_TAB)
				this->rayPicker->selectVertex(this->managerObjects->matrixProjection,
				...
				this->rayPicker->selectModel(this->managerObjects->matrixProjection,
																		 this->managerObjects->camera->matrixCamera,
																		 this->meshModelFaces,
																		 &this->rayLines,
																		 &this->sceneSelectedModelObject,
...
void Kuplung::addTerrainModel() {
	this->managerObjects->terrain->terrainGenerator->generateTerrain(
			Settings::Instance()->currentFolder,
			this->managerObjects->Setting_TerrainWidth,
			this->managerObjects->Setting_TerrainHeight);
	...
	mmf->ModelID = 1;
	mmf->init(this->managerObjects->terrain->terrainGenerator->modelTerrain,
						Settings::Instance()->currentFolder);
	mmf->initBoundingBox();
...
void Kuplung::addSpaceshipModel() {
	this->managerObjects->spaceship->spaceshipGenerator->generate(
			this->managerObjects->Setting_GridSize);
	...
	mmf->ModelID = 1;
	mmf->init(this->managerObjects->spaceship->spaceshipGenerator->modelSpaceship,
						Settings::Instance()->currentFolder);
	mmf->initBoundingBox();
	mmf->initVertexSphere();
	mmf->initModelProperties();
...
void Kuplung::renderScene() {
	...
	if(Settings::Instance()->mRayAnimate) {
		glm::vec3 rFrom = glm::vec3(Settings::Instance()->mRayOriginX,
																Settings::Instance()->mRayOriginY,
																Settings::Instance()->mRayOriginZ);
		glm::vec3 rDirection = glm::vec3(Settings::Instance()->mRayDirectionX,
																		 Settings::Instance()->mRayDirectionY,
																		 Settings::Instance()->mRayDirectionZ);
	...
	for(size_t i = 0; i < this->rayLines.size(); i++) {
		this->rayLines[i]->render(this->managerObjects->matrixProjection,
															this->managerObjects->camera->matrixCamera);
	}
...
void Kuplung::renderSceneModels() {
	...
	if(this->managerObjects->Setting_ShowTerrain) {
		...
		Settings::Instance()->sceneCountVertices +=
				static_cast<int>(this->managerObjects->terrain->terrainGenerator->vertices.size());
		Settings::Instance()->sceneCountIndices +=
				static_cast<int>(this->managerObjects->terrain->terrainGenerator->indices.size());
		Settings::Instance()->sceneCountTriangles +=
				static_cast<int>(this->managerObjects->terrain->terrainGenerator->vertices.size()) / 3;
		Settings::Instance()->sceneCountFaces +=
				static_cast<int>(this->managerObjects->terrain->terrainGenerator->vertices.size()) / 6;
	}

	if(this->managerObjects->Setting_ShowSpaceship) {
		Settings::Instance()->sceneCountObjects += 1;
		Settings::Instance()->sceneCountVertices +=
				static_cast<int>(this->managerObjects->spaceship->spaceshipGenerator->vertices.size());
		Settings::Instance()->sceneCountIndices +=
				static_cast<int>(this->managerObjects->spaceship->spaceshipGenerator->indices.size());
		Settings::Instance()->sceneCountTriangles +=
				static_cast<int>(this->managerObjects->spaceship->spaceshipGenerator->vertices.size()) / 3;
		Settings::Instance()->sceneCountFaces +=
				static_cast<int>(this->managerObjects->spaceship->spaceshipGenerator->vertices.size()) / 6;
	}

#ifdef DEF_KuplungSetting_UseCuda
	if(this->managerObjects->Setting_Cuda_ShowOceanFFT) {
		Settings::Instance()->sceneCountObjects += 1;
		this->managerUI->provider_ImGui->componentCudaExamples->exampleOceanFFT->render(
...
void Kuplung::guiProcessImportedFile(const FBEntity& file,
...
void Kuplung::processImportFileAsync(const FBEntity& file,
																		 const std::vector<std::string>& settings) {
...
void Kuplung::guiSceneExport(const FBEntity& file,
														 const std::vector<std::string>& settings,
														 ImportExportFormats exportFormat,
														 int exportFormatAssinc) {
	this->managerUI->showExporting();
	this->exporterThreadFinished = false;
	std::thread exporterThread(&Kuplung::exportSceneAsync,
														 this,
														 file,
														 this->meshModelFaces,
														 settings,
...
void Kuplung::exportSceneAsync(const FBEntity& file,
															 std::vector<ModelFaceBase*> const& meshModelFaces,
															 const std::vector<std::string>& settings,
															 ImportExportFormats exportFormat,
															 int exportFormatAssinc) {
	this->managerExporter->exportScene(
			file, meshModelFaces, settings, this->managerObjects, exportFormat, exportFormatAssinc);
	this->exporterThreadFinished = true;
}
...
void Kuplung::guiModelDelete(const int selectedModel) {
	...
	this->sceneSelectedModelObject = -1;
	this->managerUI->setSceneSelectedModelObject(-1);
	this->managerRendering->meshModelFaces.erase(this->managerRendering->meshModelFaces.begin() +
																							 selectedModel);
}
...
void Kuplung::guiRenderScene(const FBEntity& file) {
	...
	std::string renderedImage = this->imageRenderer->renderImage(
			ImageRendererType_DefaultForward, file, &this->meshModelFaces);
...
void Kuplung::openScene(const FBEntity& file) {
	...
	this->managerRendering->meshModelFaces.insert(
			this->managerRendering->meshModelFaces.begin(), begin(models), end(models));
