...
void LightRay::initBuffers(const glm::vec3& position, const glm::vec3& direction, const bool simple) {
  if ((position.x > x || position.x < x) && (position.y > y || position.y < y) && (position.z > z || position.z < z)) {
    ...
    if (simple) {
      ...
      const GLfloat g_vertex_buffer_data[] = {-0.866024971f, 0, 0.5, 0.866024971f, 0, -0.5, 0, 0, -1, 0, 0, -1, -0.866024971f, 0, -0.5, -0.866024971f, 0, 0.5, -0.866024971f, 0, 0.5, 0, 0, 1, 0.866024971f, 0, 0.5, 0.866024971f, 0, 0.5, 0.866024971f, 0, -0.5, -0.866024971f, 0, 0.5};
