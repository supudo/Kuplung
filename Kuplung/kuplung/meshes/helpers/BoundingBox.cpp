...
void BoundingBox::initBuffers(const MeshModel& meshModel) {
  ...
  // vertices
  this->dataVertices = {-0.5, -0.5, -0.5, 1.0, 0.5, -0.5, -0.5, 1.0, 0.5, 0.5, -0.5, 1.0, -0.5, 0.5, -0.5, 1.0, -0.5, -0.5, 0.5, 1.0, 0.5, -0.5, 0.5, 1.0, 0.5, 0.5, 0.5, 1.0, -0.5, 0.5, 0.5, 1.0};
  ...
  // indices
  this->dataIndices = {0, 1, 2, 3, 4, 5, 6, 7, 0, 4, 1, 5, 2, 6, 3, 7};
  ...
  for (unsigned int i = 0; i < this->meshModel.vertices.size(); i++) {
    if (this->meshModel.vertices[i].x < this->min_x)
      this->min_x = this->meshModel.vertices[i].x;
    if (this->meshModel.vertices[i].x > this->max_x)
      this->max_x = this->meshModel.vertices[i].x;
    if (this->meshModel.vertices[i].y < this->min_y)
      this->min_y = this->meshModel.vertices[i].y;
    if (this->meshModel.vertices[i].y > this->max_y)
      this->max_y = this->meshModel.vertices[i].y;
    if (this->meshModel.vertices[i].z < this->min_z)
      this->min_z = this->meshModel.vertices[i].z;
    if (this->meshModel.vertices[i].z > this->max_z)
