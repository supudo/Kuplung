...
void Dot::init(std::function<void(std::string)> doLog, std::string shaderName, int glslVersion) {
    this->doLogFunc = doLog;
    this->glUtils = new GLUtils();
    this->glUtils->init(std::bind(&Dot::doLog, this, std::placeholders::_1));
...
void Dot::initBuffers(glm::vec3 position, glm::vec3 direction, bool simple) {
...
void Dot::render(glm::mat4 matrixProjection, glm::mat4 matrixCamera, glm::mat4 matrixModel) {
    if (this->glVAO > 0) {
...
std::string Dot::readFile(const char *filePath) {
    std::string content;
...
void Dot::doLog(std::string logMessage) {
    this->doLogFunc("[Dot] " + logMessage);
}
