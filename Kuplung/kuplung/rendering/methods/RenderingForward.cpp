...
void RenderingForward::renderModels(const std::vector<ModelFaceData*>& meshModelFaces, const int& selectedModel) {
  ...
  for (ModelFaceData* mfd : meshModelFaces) {
    ...
    glm::mat4 matrixModel = this->managerObjects->grid->matrixModel * glm::translate(glm::mat4(1.0f), position) * glm::toMat4(glm::quat(rotation)) * glm::scale(glm::mat4(1.0f), scale);
