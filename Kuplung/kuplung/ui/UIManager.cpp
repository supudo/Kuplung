...
UIManager::UIManager(ObjectsManager& managerObjects)
...
void UIManager::init(SDL_Window* window,
										 ...
										 const std::function<void(FBEntity,
																							std::vector<std::string>,
																							ImportExportFormats importFormat,
																							int importFormatAssimp)>& processImportedFile,
										 ...
										 const std::function<void(FBEntity file,
																							std::vector<std::string>,
																							ImportExportFormats exportFormat,
																							int exportFormatAssimp)>& exportScene,
	...
	this->componentFileBrowser->init(
			Settings::Instance()->logFileBrowser,
			posX,
			posY,
			Settings::Instance()->frameFileBrowser_Width,
			Settings::Instance()->frameFileBrowser_Height,
			std::bind(&UIManager::dialogFileBrowserProcessFile, this, std::placeholders::_1));

	this->componentImportFile = std::make_unique<ImportFile>();
	this->componentImportFile->init(posX,
																	posY,
																	Settings::Instance()->frameFileBrowser_Width,
																	Settings::Instance()->frameFileBrowser_Height,
																	std::bind(&UIManager::dialogImporterProcessFile,
																						this,
																						std::placeholders::_1,
																						std::placeholders::_2,
																						std::placeholders::_3,
	...
	this->componentExportFile->init(posX,
																	posY,
																	Settings::Instance()->frameFileBrowser_Width,
																	Settings::Instance()->frameFileBrowser_Height,
																	std::bind(&UIManager::dialogExporterProcessFile,
																						this,
																						std::placeholders::_1,
																						std::placeholders::_2,
																						std::placeholders::_3,
	...
	this->componentFileSaver->init(posX,
																 posY,
																 Settings::Instance()->frameFileBrowser_Width,
																 Settings::Instance()->frameFileBrowser_Height,
																 std::bind(&UIManager::dialogFileSaveProcessFile,
																					 this,
																					 std::placeholders::_1,
																					 std::placeholders::_2));
	...
	this->controlsModels->init(
			this->sdlWindow, this->funcAddShape, this->funcAddLight, this->funcDeleteModel);
...
void UIManager::renderStart(bool isFrame, int* sceneSelectedModelObject) {
	...
	if(ImGui::BeginMainMenuBar()) {
			...
			if(ImGui::BeginMenu("   Import")) {
				...
				if(ImGui::BeginMenu("Assimp...")) {
					for(size_t a = 0; a < Settings::Instance()->AssimpSupportedFormats_Import.size(); a++) {
						...
						std::string f =
								std::string(format.description) + " (" + std::string(format.fileExtension) + ")";
			...
			if(ImGui::BeginMenu(ICON_FA_FILES_O " Import Recent")) {
				if(this->recentFilesImported.size() == 0)
					...
					for(size_t i = 0; i < this->recentFilesImported.size(); i++) {
						...
						if(ImGui::MenuItem(file.title.c_str(), NULL, false, true)) {
							if(boost::filesystem::exists(file.path))
								this->funcProcessImportedFile(
										file,
										std::vector<std::string>(),
										static_cast<ImportExportFormats>(this->dialogImportType),
										dialogImportType_Assimp);
							else
		...
		if(ImGui::BeginMenu("Scene")) {
			...
			if(ImGui::BeginMenu(ICON_FA_CERTIFICATE " Scene Rendering")) {
				if(ImGui::MenuItem(
							 "Solid", NULL, this->managerObjects.viewModelSkin == ViewModelSkin_Solid))
					this->managerObjects.viewModelSkin = ViewModelSkin_Solid;
				if(ImGui::MenuItem(
							 "Material", NULL, this->managerObjects.viewModelSkin == ViewModelSkin_Material))
					this->managerObjects.viewModelSkin = ViewModelSkin_Material;
				if(ImGui::MenuItem(
							 "Texture", NULL, this->managerObjects.viewModelSkin == ViewModelSkin_Texture))
					this->managerObjects.viewModelSkin = ViewModelSkin_Texture;
				if(ImGui::MenuItem(
							 "Wireframe", NULL, this->managerObjects.viewModelSkin == ViewModelSkin_Wireframe))
					this->managerObjects.viewModelSkin = ViewModelSkin_Wireframe;
				if(ImGui::MenuItem(
							 "Rendered", NULL, this->managerObjects.viewModelSkin == ViewModelSkin_Rendered))
		...
		if(ImGui::BeginMenu("View")) {
			ImGui::MenuItem(this->showControlsGUI ? ICON_FA_TOGGLE_ON " GUI Controls"
																						: ICON_FA_TOGGLE_OFF " GUI Controls",
											NULL,
											&this->showControlsGUI);
			ImGui::MenuItem(this->showControlsModels ? ICON_FA_TOGGLE_ON " Scene Controls"
																							 : ICON_FA_TOGGLE_OFF " Scene Controls",
											NULL,
											&this->showControlsModels);
			ImGui::MenuItem(Settings::Instance()->showAllVisualArtefacts
													? ICON_FA_TOGGLE_OFF " Hide Visual Artefacts"
													: ICON_FA_TOGGLE_ON " Show Visual Artefacts",
											NULL,
											&Settings::Instance()->showAllVisualArtefacts);
			ImGui::Separator();
			...
			ImGui::MenuItem(
					ICON_FA_PAPER_PLANE_O " Structured Volumetric Sampling", NULL, &this->showSVS);
	...
	if(ImGui::BeginPopupModal("Kuplung Parsing",
														&this->isParsingOpen,
														ImGuiWindowFlags_AlwaysAutoResize | ImGuiWindowFlags_NoMove |
																ImGuiWindowFlags_NoResize | ImGuiWindowFlags_NoTitleBar)) {
		ImGui::PushStyleColor(ImGuiCol_PlotHistogram,
													static_cast<ImVec4>(ImColor::HSV(0.1f / 7.0f, 0.8f, 0.8f)));
	...
	if(ImGui::BeginPopupModal("Kuplung Loading",
														&this->isLoadingOpen,
														ImGuiWindowFlags_AlwaysAutoResize | ImGuiWindowFlags_NoMove |
																ImGuiWindowFlags_NoResize | ImGuiWindowFlags_NoTitleBar)) {
	...
	if(ImGui::BeginPopupModal("Kuplung Exporting",
														&this->isExportingOpen,
														ImGuiWindowFlags_AlwaysAutoResize | ImGuiWindowFlags_NoMove |
																ImGuiWindowFlags_NoResize | ImGuiWindowFlags_NoTitleBar)) {
		ImGui::PushStyleColor(ImGuiCol_PlotHistogram,
													static_cast<ImVec4>(ImColor::HSV(0.1f / 7.0f, 0.8f, 0.8f)));
...
void UIManager::dialogFileSave(FileSaverOperation operation) {
	std::string title = "...";
	bool* wType = nullptr;
	assert(operation == FileSaverOperation_SaveScene || operation == FileSaverOperation_OpenScene ||
...
void UIManager::dialogImporterBrowser() {
	this->componentImportFile->draw(
			&this->dialogImportType, &this->dialogImportType_Assimp, &this->showImporterFile);
}

void UIManager::dialogExporterBrowser() {
	this->componentExportFile->draw(
			&this->dialogExportType, &this->dialogExportType_Assimp, &this->showExporterFile);
}
...
void UIManager::dialogShaderEditor() {
	this->componentFileEditor->draw(
			std::bind(&UIManager::fileShaderEditorSaved, this, std::placeholders::_1),
			"Editor",
			&this->showShaderEditor);
}

void UIManager::dialogKuplungIDE() {
	this->componentKuplungIDE->draw(
			"IDE", &this->showKuplungIDE, *this->meshModelFaces, this->managerObjects);
}
...
void UIManager::dialogLog() {
	...
	this->componentLog->init(
			posX, posY, Settings::Instance()->frameLog_Width, Settings::Instance()->frameLog_Height);
...
void UIManager::dialogOptions(ImGuiStyle* ref) {
	this->windowOptions->showOptionsWindow(
			ref, this->windowStyle.get(), &this->showOptions, &this->needsFontChange);
}
...
void UIManager::dialogControlsModels(int* sceneSelectedModelObject) {
	this->controlsModels->render(
			&this->showControlsModels, &this->isFrame, this->meshModelFaces, sceneSelectedModelObject);
}
...
#ifdef DEF_KuplungSetting_UseCuda
	this->componentCudaExamples->draw(&this->showCudaExamples,
...
void UIManager::dialogImporterProcessFile(FBEntity const& file,
																					std::vector<std::string> settings,
																					ImportExportFormats importFormat,
																					int importFormatAssimp) {
	this->funcProcessImportedFile(file, settings, importFormat, importFormatAssimp);
	this->showImporterFile = false;
}

void UIManager::dialogExporterProcessFile(FBEntity const& file,
																					std::vector<std::string> settings,
																					ImportExportFormats exportFormat,
																					int exportFormatAssimp) {
