...
void draw_demostuff(NVGcontext *vg, int x, int y, float w, float h) {
    ...
    x = ox;
    y += 40;
    float progress_value = fmodf((SDL_GetTicks() / 1000.0f)/10.0,1.0);
    ...
    int rw = x+240-rx;
    float s_offset = sinf((SDL_GetTicks() / 1000.0f)/2.0)*0.5+0.5;
    float s_size = cosf((SDL_GetTicks() / 1000.0f)/3.11)*0.5+0.5;
    ...
    int t = int((SDL_GetTicks() / 1000.0f)*2);
...
void draw(NVGcontext *vg, float w, float h) {
    ...
    uiProcess((int)((SDL_GetTicks() / 1000.0f)*1000.0));
}
...
static void mousebutton(SDL_Window *window, int button, int action, int mods) {
    NVG_NOTUSED(window);
    switch(button) {
    case 1: button = 2; break;
    case 2: button = 1; break;
    }
    uiSetButton(button, mods, (action==SDL_KEYDOWN)?1:0);
}

static void cursorpos(SDL_Window *window, double x, double y) {
    NVG_NOTUSED(window);
    uiSetCursor((int)x,(int)y);
}

static void scrollevent(SDL_Window *window, double x, double y) {
    NVG_NOTUSED(window);
    uiSetScroll((int)x, (int)y);
}

static void charevent(SDL_Window *window, unsigned int value) {
    NVG_NOTUSED(window);
    uiSetChar(value);
}

static void key(SDL_Window* window, int key, int scancode, int action, int mods)
{
    NVG_NOTUSED(scancode);
    if (key == SDLK_ESCAPE && action == SDL_KEYDOWN)
        printf("[OUI] CLOSE WINDOW!!!!!");
...
void OuiNanoVG_Implementation::renderStart(bool isFrame, int * sceneSelectedModelObject) {
