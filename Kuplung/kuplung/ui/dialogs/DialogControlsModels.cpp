...
void DialogControlsModels::showTextureLine(std::string const& chkLabel, MaterialTextureType texType, bool* showWindow, bool* loadTexture) {
  ...
  assert(texType == MaterialTextureType_Ambient || texType == MaterialTextureType_Diffuse || texType == MaterialTextureType_Dissolve || texType == MaterialTextureType_Bump || texType == MaterialTextureType_Specular || texType == MaterialTextureType_SpecularExp || texType == MaterialTextureType_Displacement);
  switch (texType) {
...
void DialogControlsModels::showTextureImage(ModelFaceBase* mmf, MaterialTextureType type, std::string title, bool* showWindow, bool* genTexture, GLuint* vboBuffer, int* width, int* height) const {
...
void DialogControlsModels::drawModels(bool* isFrame, std::vector<ModelFaceBase*>* mmfaces) {
  ...
  if (this->selectedObject > -1 && sizeof(this->meshModelFaces) > static_cast<size_t>(this->selectedObject)) {
    ...
    ImGui::SameLine();
    ImGui::Text("%s", mmf->meshModel.File.title.c_str());
    ImGui::TextColored(ImVec4(255, 0, 0, 255), "ModelFace:");
    ImGui::SameLine();
    ImGui::Text("%s", mmf->meshModel.ModelTitle.c_str());
    ImGui::TextColored(ImVec4(255, 0, 0, 255), "Material:");
    ImGui::SameLine();
    ImGui::Text("%s", mmf->meshModel.MaterialTitle.c_str());
    ImGui::TextColored(ImVec4(255, 0, 0, 255), "Vertices:");
    ImGui::SameLine();
    ImGui::Text("%i", mmf->meshModel.countVertices);
    ImGui::TextColored(ImVec4(255, 0, 0, 255), "Normals:");
    ImGui::SameLine();
    ImGui::Text("%i", mmf->meshModel.countNormals);
    ImGui::TextColored(ImVec4(255, 0, 0, 255), "Indices:");
    ImGui::SameLine();
    ImGui::Text("%i", mmf->meshModel.countIndices);

    if (!mmf->meshModel.ModelMaterial.TextureAmbient.Image.empty() || !mmf->meshModel.ModelMaterial.TextureDiffuse.Image.empty() || !mmf->meshModel.ModelMaterial.TextureBump.Image.empty() || !mmf->meshModel.ModelMaterial.TextureDisplacement.Image.empty() || !mmf->meshModel.ModelMaterial.TextureDissolve.Image.empty() || !mmf->meshModel.ModelMaterial.TextureSpecular.Image.empty() || !mmf->meshModel.ModelMaterial.TextureSpecularExp.Image.empty()) {
      ImGui::Separator();
    ...
    switch (this->selectedTabScene) {
      ...
      case 7: {
        ...
        const char* illum_models_items[] = {"[0] Color on and Ambient off", // "Shaderless" option in Blender, under "Shading" in Material tab
...
void DialogControlsModels::drawCreate() {
  ...
  if (ImGui::Button("Triangle", ImVec2(-1, 0)))
    this->funcAddShape(ShapeType_Triangle);
  if (ImGui::Button("Cone", ImVec2(-1, 0)))
    this->funcAddShape(ShapeType_Cone);
  if (ImGui::Button("Cube", ImVec2(-1, 0)))
    this->funcAddShape(ShapeType_Cube);
  if (ImGui::Button("Cylinder", ImVec2(-1, 0)))
    this->funcAddShape(ShapeType_Cylinder);
  if (ImGui::Button("Grid", ImVec2(-1, 0)))
    this->funcAddShape(ShapeType_Grid);
  if (ImGui::Button("Ico Sphere", ImVec2(-1, 0)))
    this->funcAddShape(ShapeType_IcoSphere);
  if (ImGui::Button("Plane", ImVec2(-1, 0)))
    this->funcAddShape(ShapeType_Plane);
  if (ImGui::Button("Torus", ImVec2(-1, 0)))
    this->funcAddShape(ShapeType_Torus);
  if (ImGui::Button("Tube", ImVec2(-1, 0)))
    this->funcAddShape(ShapeType_Tube);
  if (ImGui::Button("UV Sphere", ImVec2(-1, 0)))
    this->funcAddShape(ShapeType_UVSphere);
  if (ImGui::Button("Monkey Head", ImVec2(-1, 0)))
    this->funcAddShape(ShapeType_MonkeyHead);

  ImGui::Separator();
  ImGui::Separator();

  if (ImGui::Button("Epcot", ImVec2(-1, 0)))
    this->funcAddShape(ShapeType_Epcot);
  if (ImGui::Button("Brick Wall", ImVec2(-1, 0)))
    this->funcAddShape(ShapeType_BrickWall);
  if (ImGui::Button("Plane Objects", ImVec2(-1, 0)))
    this->funcAddShape(ShapeType_PlaneObjects);
  if (ImGui::Button("Plane Objects - Large Plane", ImVec2(-1, 0)))
    this->funcAddShape(ShapeType_PlaneObjectsLargePlane);
  if (ImGui::Button("Material Ball", ImVec2(-1, 0)))
    this->funcAddShape(ShapeType_MaterialBall);
  if (ImGui::Button("Material Ball - Blender", ImVec2(-1, 0)))
    this->funcAddShape(ShapeType_MaterialBallBlender);

  ImGui::Separator();
  ImGui::Separator();

  if (ImGui::Button("Directional (Sun)", ImVec2(-1, 0)))
    this->funcAddLight(LightSourceType_Directional);
  if (ImGui::Button("Point (Light bulb)", ImVec2(-1, 0)))
    this->funcAddLight(LightSourceType_Point);
  if (ImGui::Button("Spot (Flashlight)", ImVec2(-1, 0)))
    this->funcAddLight(LightSourceType_Spot);
}
