...
void UVEditor::init(int positionX, int positionY, int width, int height) {
  ...
  this->componentFileBrowser->init(Settings::Instance()->logFileBrowser, 50, 50, Settings::Instance()->frameFileBrowser_Width, Settings::Instance()->frameFileBrowser_Height, std::bind(&UVEditor::dialogFileBrowserProcessFile, this, std::placeholders::_1, std::placeholders::_2));
  this->componentFileBrowser->setImageBrowser(true);
}

void UVEditor::setModel(ModelFaceBase* mmf, MaterialTextureType texType, std::string const& texturePath, const std::function<void(ModelFaceBase*)>& processTexture) {
  this->mmf = mmf;
...
void UVEditor::draw(const char* title, bool* p_opened) {
  ...
  const char* mapping_items[] = {"-- Select Unwrapping Method --", "Square", "Cube", "Cylinder", "Sphere"};
...
void UVEditor::processTextureCoordinatesSquare() const {
  if (this->uvPoints.size() > 0) {
...
void UVEditor::processTextureCoordinates() const {
  switch (this->uvUnwrappingType) {
    case UVUnwrappingMethod_Default:
