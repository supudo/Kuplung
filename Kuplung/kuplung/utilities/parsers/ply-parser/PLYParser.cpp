...
std::vector<MeshModel> PLYParser::parse(const FBEntity& file, const std::vector<std::string>& settings) {
    ...
    while (std::getline(ply_file_data, singleLine)) {
        if (boost::starts_with(singleLine, "format")) {
            boost::replace_first(singleLine, "format ", "");
            if (boost::starts_with(singleLine, "ascii")) {
...
void PLYParser::loadAsciiFile(const FBEntity& file) {
    ...
    while (std::getline(ply_file_data, singleLine)) {
        // elements
        if (boost::starts_with(singleLine, "element")) {
            boost::replace_first(singleLine, "element ", "");

            // vertices
            if (boost::starts_with(singleLine, "vertex")) {
            ...
            // faces
            if (boost::starts_with(singleLine, "face")) {
                boost::replace_first(singleLine, "face ", "");
        ...
        // vertex properties
        if (boost::starts_with(singleLine, "property") && inElementVertex) {
            boost::replace_first(singleLine, "property ", "");
        ...
        if (boost::starts_with(singleLine, "end_header"))
            headerEnded = true;
    }
    ply_file_data.close();
