...
struct MeshModel {
    ...
    const unsigned int dataCountVertices() const noexcept { return countVertices * sizeof(glm::vec3); }
    const unsigned int dataCountTextureCoordinates() const noexcept { return countTextureCoordinates * sizeof(glm::vec2); }
    const unsigned int dataCountNormals() const noexcept { return countNormals * sizeof(glm::vec3); }
    const unsigned int dataCountIndices() const noexcept { return countIndices; }
    const unsigned int dataCount() const noexcept { return dataCountVertices() + dataCountTextureCoordinates() + dataCountNormals() + dataCountIndices(); }
    const unsigned int dataCountBillboard(const bool includeColors) const noexcept { return dataCountVertices() + (includeColors ? dataCountVertices() : 0) + dataCountIndices(); }
};
