...
* If there is not enough information, -1 is returned. If the method is recalled to quickly, the previous value is returned.
***********************************************/
short WindowsCPUUsage::GetUsage() {
  ...
  if (InterlockedIncrement(&m_lRunCount) == 1) {
    ...
    if (!IsFirstRun()) {
      // CPU usage is calculated by getting the total amount of time the system has operated since the last measurement
      // (made up of kernel + user) and the total amount of time the process has run (kernel + user).
...
ULONGLONG WindowsCPUUsage::SubtractTimes(const FILETIME& ftA, const FILETIME& ftB) {
  LARGE_INTEGER a, b;
...
bool WindowsCPUUsage::EnoughTimePassed() {
