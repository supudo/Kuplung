...
GLuint oceanFFT0::draw(glm::mat4 matrixProjection, glm::mat4 matrixCamera, glm::mat4 matrixGrid) {
  this->timerEvent();
...
void oceanFFT0::renderCuda(glm::mat4 matrixProjection, glm::mat4 matrixCamera, glm::mat4 matrixGrid) {
...
bool oceanFFT0::runAutoTest() {
  ...
  // create FFT plan
  checkCudaErrors(cufftPlan2d(&this->fftPlan, this->meshSize, this->meshSize, CUFFT_C2C));

  // allocate memory
  int spectrumSize = this->spectrumW * this->spectrumH * sizeof(float2);
  checkCudaErrors(cudaMalloc((void **)&this->d_h0, spectrumSize));
  this->h_h0 = (float2 *) malloc(spectrumSize);
  generate_h0(this->h_h0);
  checkCudaErrors(cudaMemcpy(this->d_h0, this->h_h0, spectrumSize, cudaMemcpyHostToDevice));

  int outputSize =  this->meshSize * this->meshSize * sizeof(float2);
  checkCudaErrors(cudaMalloc((void **)&this->d_ht, outputSize));
  checkCudaErrors(cudaMalloc((void **)&this->d_slope, outputSize));

  sdkCreateTimer(&this->timer);
  sdkStartTimer(&this->timer);
  this->prevTime = sdkGetTimerValue(&this->timer);

  runCudaTest();

  checkCudaErrors(cudaFree(this->d_ht));
  checkCudaErrors(cudaFree(this->d_slope));
  checkCudaErrors(cudaFree(this->d_h0));
  checkCudaErrors(cufftDestroy(this->fftPlan));
  free(this->h_h0);

  return result;
}

void oceanFFT0::runCudaTest() {
  checkCudaErrors(cudaMalloc((void **)&this->g_hptr, this->meshSize * this->meshSize * sizeof(float)));
  checkCudaErrors(cudaMalloc((void **)&this->g_sptr, this->meshSize * this->meshSize * sizeof(float2)));
  ...
  checkCudaErrors(cufftExecC2C(this->fftPlan, this->d_ht, this->d_ht, CUFFT_INVERSE));

  std::string fpath = Settings::Instance()->appFolder() + "/cuda_data/";
  std::vector<char> fpath_cstr(fpath.c_str(), fpath.c_str() + fpath.size() + 1);
  ...
  checkCudaErrors(cudaFree(this->g_hptr));
  checkCudaErrors(cudaFree(this->g_sptr));
}

void oceanFFT0::runGraphicsTest() {
...
void oceanFFT0::runCuda() {
  ...
  checkCudaErrors(cudaGraphicsMapResources(1, &this->cuda_heightVB_resource, 0));
  checkCudaErrors(cudaGraphicsResourceGetMappedPointer((void **)&this->g_hptr, &num_bytes, this->cuda_heightVB_resource));

  cudaUpdateHeightmapKernel(this->g_hptr, this->d_ht, this->meshSize, this->meshSize);

  // calculate slope for shading
  checkCudaErrors(cudaGraphicsMapResources(1, &this->cuda_slopeVB_resource, 0));
  checkCudaErrors(cudaGraphicsResourceGetMappedPointer((void **)&this->g_sptr, &num_bytes, this->cuda_slopeVB_resource));

  cudaCalculateSlopeKernel(this->g_hptr, this->g_sptr, this->meshSize, this->meshSize);

  checkCudaErrors(cudaGraphicsUnmapResources(1, &this->cuda_heightVB_resource, 0));
  checkCudaErrors(cudaGraphicsUnmapResources(1, &this->cuda_slopeVB_resource, 0));
}

void oceanFFT0::timerEvent() {
  float time = sdkGetTimerValue(&this->timer);
...
// V - wind speed
// A - constant
float oceanFFT0::phillips(float Kx, float Ky, float Vdir, float V, float A, float dir_depend) {
...
void oceanFFT0::generate_h0(float2 *h0) {
...
void oceanFFT0::initBuffers() {
  ...
  checkCudaErrors(cudaGraphicsGLRegisterBuffer(&this->cuda_heightVB_resource, this->heightVertexBuffer, cudaGraphicsMapFlagsWriteDiscard));
  ...
  checkCudaErrors(cudaGraphicsGLRegisterBuffer(&this->cuda_slopeVB_resource, this->slopeVertexBuffer, cudaGraphicsMapFlagsWriteDiscard));
...
bool oceanFFT0::initShaderProgram() {
...
void oceanFFT0::generateAttachmentTexture(GLboolean depth, GLboolean stencil) {
...
oceanFFT0::~oceanFFT0() {
  sdkDeleteTimer(&this->timer);
  checkCudaErrors(cudaGraphicsUnregisterResource(this->cuda_heightVB_resource));
  checkCudaErrors(cudaGraphicsUnregisterResource(this->cuda_slopeVB_resource));
